// Asynchronous function to fetch and populate the gold membership business directory
async function populateGoldBusinessDirectory() {
  try {
    ...
    // Filter businesses by the 'gold' membership
    const goldBusinesses = businesses.filter(
      (business) => business.membership === "Gold"
    );

    // Get the directory container element using the class and ID you provided
    const directoryElement = document.querySelector(
      ".directory#businessDirectory"
    );

    // Loop through each gold business and display it
    goldBusinesses.forEach((business) => {
      ...
      // Add the business details (email, phone, URL, and membership level)
      ...
      membership.innerHTML = `<span><strong>MEMBERSHIP: </strong></span>${business.membership}`;

      // Append everything to the business details section
      businessDetails.appendChild(email);
      businessDetails.appendChild(phone);
      businessDetails.appendChild(website);
      businessDetails.appendChild(membership);
      ...
      // Append the business div to the business directory
...
// Call the function to populate the gold business directory
populateGoldBusinessDirectory();
